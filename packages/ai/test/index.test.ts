...
test("founder calibration setup report gives role-specific handoff links", () => {
  ...
  assert.match(report.requiredRoles.maria.handoffText, /Continue development with the next guided journal pass/)
  assert.equal(JSON.stringify(report).includes("private journal text"), false)
  assert.equal(JSON.stringify(report).includes("raw note"), false)
})
...
test("founder handoff report resolves copyable web and admin links", () => {
  ...
  assert.equal(maria?.primaryHref, "https://web.example/login?email=maria%40example.com&next=%2Fjournal")
  assert.match(maria?.handoffText ?? "", /https:\/\/web\.example\/login\?email=maria%40example\.com&next=%2Fjournal/)
  ...
  assert.match(packet, /https:\/\/web\.example\/login\?email=maria%40example\.com&next=%2Fjournal/)
