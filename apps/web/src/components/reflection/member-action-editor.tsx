...
export function MemberActionEditor({ councilSessionId, initialActions, labels }: {
  ...
  async function save() {
    ...
    try {
      ...
      setActions((current) => [...current, { id: payload.response.id, text: text.trim() }])
      setText("")
      setState("saved")
    } catch {
      setState("error")
  ...
  return (
    <div>
      ...
      {actions.length > 0 && (
        <ul className="mt-4 space-y-2" aria-label={labels.saved}>
          {actions.map((action) => <li key={action.id} className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface)] px-4 py-3 text-sm text-[var(--text-primary)]">{action.text}</li>)}
        </ul>
      )}
      <textarea value={text} onChange={(event) => { setText(event.target.value); setState("idle") }} maxLength={1000} placeholder={labels.placeholder} className="mt-4 min-h-24 w-full resize-y rounded-xl border border-[var(--border-subtle)] bg-[var(--surface)] px-4 py-3 text-sm text-[var(--text-primary)] outline-none" />
